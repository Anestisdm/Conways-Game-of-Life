...
def nearest_location(loc, loc_list, loc_type=None):
    ...
    while i<len(loc_list):
        if distance(loc,loc_list[i])<min and loc!=loc_list[i]:
            if loc_type==None:
                min=distance(loc,loc_list[i])
                min_name=loc_list[i]
            elif loc_type==type(loc_list[i]):
                min=distance(loc,loc_list[i])
                min_name=loc_list[i]
        i+=1
    return min_name
...
def flatten(field):
    ...
    """GRAPSTE TON KWDIKA SAS APO KATW."""
    result=[]
    field_copy=[x for x in field]
    result.append(field_copy[0])
    while field_copy[-1]!=None:
        field_copy=field_copy[1]
        result.append(field_copy[0])
...
def cherry_string(field):
    ...
    return reduce(add, [x if 'cherry' in x else '' for x in flatten(field)])
